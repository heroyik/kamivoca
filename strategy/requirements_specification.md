...
- **주요 전략**: 기존 스페인어 학습 앱(`HolaVoca`)의 아키텍처를 기반으로 하되, 데이터베이스, 클라우드 리소스 및 서드파티 API를 완전히 분리하여 독립적으로 운영함.
...
- **Firebase (인증/DB)**: 기존 HolaVoca 인스턴스와 완전히 분리된 신규 프로젝트 설정(완전 분리원칙 준수).
- **데이터 스키마**:
